    ...
    try {
      ...
      while (true) {
        ...
        if (request.startsWith("login")) {
          try {
            currentAccount =
                program.getAccount(request.split(" ")[1].trim(), request.split(" ")[2].trim());
          ...
          if (program.addAccount(data[2], data[3])) pr.println("User added");
          else pr.println("User already exists");
          ...
          try {
            if (playlist.remove(
                chooseSong(request.substring(request.indexOf(" ", space)).trim(), pr, bf))) {
              pr.println("Song removed");
            } else {
          ...
          if (choice != null) {
            ...
            Thread splThread = new Thread(spl);
            splThread.start();
            pr.println(
                "Now playing:  "
                    + choice.getName()
          ...
          try {
            if (playlist.addSong(chooseSong(info[2], pr, bf))) {
              pr.println("Song added");
            } else {
              throw new NullPointerException();
            }
          } catch (NullPointerException e) {
            pr.println("Song not added");
          }
